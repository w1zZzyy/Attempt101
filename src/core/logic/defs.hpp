...
constexpr int MAX_REPETITIONS = 3;
...
public:
	...
	constexpr Castle& add(CastleRightsType _cr) noexcept {
		cr = static_cast<CastleRightsType>(cr | _cr);
		return *this;
	}
	constexpr Castle& rem(CastleRightsType _cr) noexcept {
		cr = static_cast<CastleRightsType>(cr & ~_cr);
		return *this;
	}

	constexpr CastleRightsType extract(Color c, CastleType ct) const noexcept {	
		return static_cast<CastleRightsType>(cr & (ct << (c + 1))); 
	}

	constexpr bool available(CastleRightsType _cr) const noexcept {
		return (cr & _cr) == _cr;
	}
