...
public:
    ...
    static constexpr SquareType ToSquare(std::string_view index) {
        auto file = index[0] - 'a';
        auto rank = index[1] - '1';
    ...
    constexpr Square& operator = (const Square& s) noexcept {index = s.index; return *this;}
    constexpr Square& operator = (SquareType s) noexcept {index = s; return *this;}

    constexpr Square& operator ++ () noexcept {index = ToSquare(index + 1); return *this;}
    constexpr Square& operator -- () noexcept {index = ToSquare(index - 1); return *this;}
    ...
    constexpr bool operator == (const Square& s) const noexcept {return index == s.index;}
    constexpr bool operator != (const Square& s) const noexcept {return index == s.index;}
    constexpr bool operator == (SquareType st) const noexcept {return index == st;}
    constexpr bool operator != (SquareType st) const noexcept {return index != st;}


    constexpr Square operator + (int i) const {return ToSquare(index + i);}
    constexpr Square operator - (int i) const {return ToSquare(index - i);}
    constexpr Square& operator += (int i) {index = SquareType(index + i); return *this;}
    constexpr Square& operator -= (int i) {index = SquareType(index - i); return *this;}
    ...
    constexpr int rank() const noexcept {return index / 8;}
    constexpr int file() const noexcept {return index % 8;}
...
std::ostream& operator << (std::ostream& out, const Core::Logic::Square& sqr);
